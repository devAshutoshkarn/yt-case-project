...
document.querySelector("#btninfo").addEventListener("click", async function () {
    ...
    document.getElementById("totvid").innerHTML = "TOTAL NUMBER OF VIDEOS:-" + ' ' + `<b style="color:red" >${lines.length
        }</b>`;
    finalData = [];
    for (const item of lines) {
        ...
        document.getElementById("fatchedview").innerHTML += `<span style="color:#3fff00; font-size:18px;" >${resData.videoDetails.ownerChannelName
            }</span>` + "," + " " + `<a href="${item}" target="_blank">${item}</a>` + "," + " " + `<span style="color:white; font-size:18px;" >(${numeral(viewsreslt).format('0.0a')
            })</span>` + "," + " " + `<b style="color:yellow">${resData.videoDetails.publishDate
            }</b>` + '\n';
        finalData.push(viewsreslt);
        var fatchedviewlength = document.querySelectorAll("#fatchedview a").length;
            document.getElementById('succfeth').innerHTML=fatchedviewlength+'/'+lines.length;
    ...
    document.getElementById('rangeview22').innerHTML = "Number of Videos:-" + ' ' + `<b style="color:black">${fltview.length
        }</b>`;
